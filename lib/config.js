      ...
      pipeline: [
        {
          $match: {
            $expr: {
              //yasab olingan let variables orqali match qismda bir document ichida fieldlarni compare qiladi.
              $and: [
                //performs a logical AND operation on an array of one or more expressions
                { $eq: ["$follow_id", "$$lc_follow_id"] }, //return boolen
                { $eq: ["$subscriber_id", "$$lc_subscriber_id"] }, //$$ biz hosil qilganimiz un 2 ta qoyiladi.
              ],
            },
          },
        },
      ...
      pipeline: [
        ...
        },
        {
          $project: {
            _id: 0,
            mb_id: 1,
            like_ref_id: 1, //projection ichiga ol
